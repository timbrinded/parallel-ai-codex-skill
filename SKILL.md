...
description: Comprehensive Parallel.ai Search API and Task API implementation and debugging guidance. Use when Codex needs to build, inspect, troubleshoot, or refactor integrations using parallel.ai / Parallel / parallel-web, Search API (/v1beta/search), Task API runs (/v1/tasks/runs), task groups/events (/v1beta/tasks/groups), SSE streams, webhooks, TaskSpec structured outputs, SourcePolicy filtering, processor selection, or Parallel beta headers.
---

# Parallel.ai Search + Task API Skill

Use this skill to implement or debug Parallel.ai web search and task-run integrations.
...
1. Identify whether the user needs `search`, `task run`, `task group`, `sse`, or `webhook` behavior.
2. Confirm SDK vs raw HTTP preference (Python, TypeScript, or cURL).
3. Choose processor/mode before writing payloads.
4. Draft payload and validate locally with `scripts/validate_search_payload.py` or `scripts/validate_task_payload.py`.
...
- Mark beta-gated fields explicitly in examples (`enable_events`, `mcp_servers`, `webhook`, Search beta endpoint).
...
- Search API is currently a beta endpoint (`/v1beta/search`).
